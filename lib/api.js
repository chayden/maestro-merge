...
class SwimTopiaAPI {
  ...
  // Batch update judged heat places for event records
  async updateJudgedHeatPlaces(placeUpdates, options = {}) {
    const batchId = this.generateId();

    const records = placeUpdates.map(({ recordId, judgedHeatPlace }) => ({
      type: 'eventRecord',
      id: recordId,
      attributes: { judgedHeatPlace },
    }));
    ...
    const result = await this.request(`/meets/${this.meetId}/event-record-batches`, {
      method: 'POST',
      body: JSON.stringify(payload),
    });
    ...
    const returnedRecords = [
      ...(result?.included || []),
      ...(Array.isArray(result?.data?.attributes?.records) ? result.data.attributes.records : []),
    ];
    if (result?.data?.type === 'eventRecord') returnedRecords.push(result.data);

    const results = placeUpdates.map(({ recordId, judgedHeatPlace }) => {
      const recordResult = returnedRecords.find((record) => record.type === 'eventRecord' && record.id === recordId);
      const updatedPlace = recordResult?.attributes?.judgedHeatPlace;
      const responseIncludesPlace = !!recordResult &&
        Object.prototype.hasOwnProperty.call(recordResult.attributes || {}, 'judgedHeatPlace');
      const responseMatches = !responseIncludesPlace || Number(updatedPlace) === Number(judgedHeatPlace);

      return {
        success: responseMatches,
        recordId,
        judgedHeatPlace: responseIncludesPlace ? updatedPlace : judgedHeatPlace,
      };
    });
    ...
    if (typeof options?.onProgress === 'function') {
      options.onProgress({
        completed: placeUpdates.length,
        total: placeUpdates.length,
      });
    }
